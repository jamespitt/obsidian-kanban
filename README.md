...
- **Right-clicking a card** offers **Create note from card**: makes a new note (title derived from the task, optionally from a template - see Settings), and links the task to it by inserting `[[Note Title]]` right after the task's title text - the same `Title   [[Project Note]]` convention already used throughout the vault. The task line's tags and fields are otherwise untouched.
